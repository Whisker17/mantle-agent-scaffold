...
- `critical`
  - Allowance equals or is effectively near-max integer approval.
  - Typical near-max detection: value >= `2^255`.

## Reporting rules

- Always include rationale text with each risk label.
- Mark spender trust status as `unknown` unless verified from trusted registry or user-confirmed protocol.
