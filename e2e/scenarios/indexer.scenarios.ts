...
export const indexerScenarios: AgentScenario[] = [
  {
    id: "indexer-querySubgraph-basic",
    module: "indexer",
    toolName: "mantle_querySubgraph",
    prompt:
      "Query the Agni subgraph at {E2E_SUBGRAPH_ENDPOINT} to get the top 5 pools by TVL. Use the GraphQL query: { pools(first: 5, orderBy: totalValueLockedUSD) { id totalValueLockedUSD } }",
  ...
  },
  {
    ...
    prompt:
      "Run a SQL query against the indexer at {E2E_SQL_ENDPOINT} to get the top 10 token transfers: SELECT * FROM transfers ORDER BY block_number DESC LIMIT 10",
